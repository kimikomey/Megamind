...
void* List::Get(int pos)
{
	...
	void* data = Allocator::Instance().GetMemory(sizeof(elementSize));
	Take(pos, data);
	return data;
}
...
List::Segment* List::NewSegment()
{
	...
	try
	{
		temp->data = Allocator::Instance().GetMemory(elementCount * elementSize);
	}
	catch (bad_alloc)
	{
		error = true;
...
void List::DeleteSegment(Segment* segment)
{
	...
	Allocator::Instance().FreeMemory(segment->data);
	segmentCount--;
}
...
{
	void* temp = Allocator::Instance().GetMemory(elementSize);
